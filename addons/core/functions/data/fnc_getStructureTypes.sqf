#include "..\..\script_component.hpp"
/*
 * Function: DSC_core_fnc_getStructureTypes
 * Description:
 *     Returns curated lists of structure class names for two classification systems:
 *
 *     1. GARRISON CLASSIFICATION (main/side) — how AI uses the building
 *        Main = anchor buildings for garrison clusters
 *        Side = satellite buildings attached to anchors
 *
 *     2. FUNCTIONAL CLASSIFICATION — what the building IS
 *        Residential, commercial, industrial, etc. for location tagging
 *        and mission-type-to-location matching.
 *
 *     Also includes non-occupiable structure types that characterize a location
 *     without providing garrison positions (fuel tanks, comm towers, silos, etc.)
 *
 * Arguments:
 *     None
 *
 * Return Value:
 *     <HASHMAP> - Keys:
 *       "main"                 - Array of main structure class names
 *       "side"                 - Array of side structure class names
 *       "mainMilitary"         - Military main structures
 *       "mainCivilian"         - Civilian main structures
 *       "sideMilitary"         - Military side structures
 *       "sideCivilian"         - Civilian side structures
 *       "exclusions"           - Per-map exclusion list
 *       "functionalCategories" - Hashmap of category -> class name array
 *       "functionalLookup"     - Hashmap of class name -> category (reverse lookup)
 *       "nonOccupiable"        - Hashmap of category -> non-enterable class arrays
 *       "nonOccupiableLookup"  - Hashmap of class name -> category (reverse lookup)
 *
 * Example:
 *     private _types = call DSC_core_fnc_getStructureTypes;
 *     private _mainTypes = _types get "main";
 *     private _funcLookup = _types get "functionalLookup";
 */

// ============================================================================
// MAIN STRUCTURES (Anchor buildings - multi-story, barracks, HQs, large buildings)
// ============================================================================

// --- Military Main ---
private _mainMilitary = [
    "Land_i_Barracks_V1_F",
    "Land_i_Barracks_V2_F",
    "Land_u_Barracks_V2_F",
    "Land_Barracks_01_camo_F",
    "Land_Barracks_01_grey_F",
    "Land_Barracks_01_dilapidated_F",
    "Land_Barracks_02_F",
    "Land_Barracks_03_F",
    "Land_Barracks_04_F",
    "Land_Barracks_05_F",
    "Land_Barracks_06_F",
    "Cargo_HQ_base_F",
    "Land_MilOffices_V1_F",
    "Land_Research_house_V1_F",
    "Land_Research_HQ_F",
    "Land_Bunker_01_big_F",
    "Land_Bunker_01_HQ_F",
    "Land_Bunker_01_tall_F",
    "Land_PillboxBunker_01_big_F",
    "Land_PillboxBunker_01_rectangle_F",
    "Land_PillboxBunker_01_hex_F",
    "Land_Medevac_HQ_V1_F",
    "Land_Airport_Tower_F",
    "Land_Airport_01_controlTower_F",
    "Land_Airport_02_controlTower_F",
    "Land_Radar_F",
    "Land_Radar_01_HQ_F",
    "Land_Radar_01_antenna_base_F",
    "Land_Radar_01_kitchen_F",
    "Land_Cargo_HQ_V3_derelict_F",
    "Land_ControlTower_01_F",
    "Land_ControlTower_02_F",
    "Land_Hangar_F",
    "Land_i_Barracks_V1_dam_F",
    "Land_i_Barracks_V2_dam_F",
    "Land_MedicalTent_01_tropic_closed_F",
    "Land_MedicalTent_01_MTP_closed_F",
    "Land_MedicalTent_01_greenhex_closed_F",
    "Land_MedicalTent_01_brownhex_closed_F",
    "Land_MedicalTent_01_digital_closed_F",
    "Land_MedicalTent_01_white_IDAP_med_closed_F",
    "Land_MedicalTent_01_white_generic_closed_F",
    "Land_MedicalTent_01_white_generic_open_F",
    "Land_MedicalTent_01_white_generic_outer_F",
    "Land_MedicalTent_01_white_generic_inner_F",
    "Land_MedicalTent_01_white_IDAP_closed_F",
    "Land_MedicalTent_01_white_IDAP_open_F",
    "Land_MedicalTent_01_white_IDAP_outer_F",
    // Livonia
    "Land_DeconTent_01_white_F",
    "Land_DeconTent_01_yellow_F",
    "Land_DeconTent_01_IDAP_F",
    "Land_DeconTent_01_CSAT_brownhex_F",
    "Land_DeconTent_01_CSAT_greenhex_F",
    "Land_DeconTent_01_NATO_F",
    "Land_DeconTent_01_NATO_tropic_F",
    "Land_DeconTent_01_AAF_F",
    "Land_MedicalTent_01_aaf_generic_inner_F",
    "Land_MedicalTent_01_aaf_generic_closed_F",
    "Land_MedicalTent_01_aaf_generic_open_F",
    "Land_MedicalTent_01_aaf_generic_outer_F",
    "Land_MedicalTent_01_NATO_generic_inner_F",
    "Land_MedicalTent_01_NATO_generic_closed_F",
    "Land_MedicalTent_01_NATO_generic_open_F",
    "Land_MedicalTent_01_NATO_generic_outer_F",
    "Land_MedicalTent_01_NATO_tropic_generic_inner_F",
    "Land_MedicalTent_01_NATO_tropic_generic_closed_F",
    "Land_MedicalTent_01_NATO_tropic_generic_open_F",
    "Land_MedicalTent_01_NATO_tropic_generic_outer_F",
    "Land_MedicalTent_01_CSAT_brownhex_generic_inner_F",
    "Land_MedicalTent_01_CSAT_brownhex_generic_closed_F",
    "Land_MedicalTent_01_CSAT_brownhex_generic_open_F",
    "Land_MedicalTent_01_CSAT_brownhex_generic_outer_F",
    "Land_MedicalTent_01_CSAT_greenhex_generic_inner_F",
    "Land_MedicalTent_01_CSAT_greenhex_generic_closed_F",
    "Land_MedicalTent_01_CSAT_greenhex_generic_open_F",
    "Land_MedicalTent_01_CSAT_greenhex_generic_outer_F"
];

// --- Civilian Main ---
private _mainCivilian = [
    "Land_i_House_Big_01_V1_F",
    "Land_i_House_Big_01_V2_F",
    "Land_i_House_Big_01_V3_F",
    "Land_i_House_Big_02_V1_F",
    "Land_i_House_Big_02_V2_F",
    "Land_i_House_Big_02_V3_F",
    "Land_Offices_01_V1_F",
    "Land_i_House_Big_01_b_yellow_F",
    "Land_i_House_Big_01_b_blue_F",
    "Land_i_House_Big_01_b_pink_F",
    "Land_i_House_Big_01_b_white_F",
    "Land_i_House_Big_01_b_whiteblue_F",
    "Land_i_House_Big_01_b_brown_F",
    "Land_i_House_Big_02_b_pink_F",
    "Land_i_House_Big_02_b_brown_F",
    "Land_i_House_Big_02_b_whiteblue_F",
    "Land_i_House_Big_02_b_blue_F",
    "Land_i_House_Big_02_b_yellow_F",
    "Land_i_House_Big_02_b_white_F",
    "Land_u_House_Big_01_V1_F",
    "Land_u_House_Big_02_V1_F",
    "Land_i_Stone_HouseBig_V1_F",
    "Land_i_Stone_HouseBig_V2_F",
    "Land_i_Stone_HouseBig_V3_F",
    "Land_i_Stone_House_Big_01_b_clay_F",
    "Land_i_House_Big_01_V1_dam_F",
    "Land_i_House_Big_02_V1_dam_F",
    "Land_d_House_Big_01_V1_F",
    "Land_d_House_Big_02_V1_F",
    "Land_i_Stone_HouseBig_V1_dam_F",
    "Land_d_Stone_HouseBig_V1_F",
    "Land_House_Big_01_F",
    "Land_House_Big_02_F",
    "Land_House_Big_03_F",
    "Land_House_Big_04_F",
    "Land_House_Big_05_F",
    "Land_House_2W01_F",
    "Land_House_2W02_F",
    "Land_House_2W03_F",
    "Land_House_2W04_F",
    "Land_House_2B01_F",
    "Land_House_2B02_F",
    "Land_House_2B03_F",
    "Land_House_2B04_F",
    "Land_HouseRuin_Big_01_F",
    "Land_HouseRuin_Big_01_half_F",
    "Land_HouseRuin_Big_02_F",
    "Land_HouseRuin_Big_02_half_F",
    "Land_HouseRuin_Big_03_half_F",
    "Land_GarageRow_01_large_F",
    "Land_GarageOffice_01_F",
    "Land_i_Shop_01_V1_F",
    "Land_i_Shop_01_V2_F",
    "Land_i_Shop_01_V3_F",
    "Land_i_Shop_02_V1_F",
    "Land_i_Shop_02_V2_F",
    "Land_i_Shop_02_V3_F",
    "Land_u_Shop_01_V1_F",
    "Land_u_Shop_02_V1_F",
    "Land_Shop_City_01_F",
    "Land_Shop_City_02_F",
    "Land_Shop_City_05_F",
    "Land_Shop_City_06_F",
    "Land_Shop_City_07_F",
    "Land_Shop_Town_01_F",
    "Land_Shop_Town_03_F",
    "Land_Shop_Town_05_F",
    "Land_Shop_Town_02_F",
    "Land_Shop_01_V1_ruins_F",
    "Land_Shop_02_V1_ruins_F",
    "Land_Shop_City_04_F",
    "Land_i_Shop_01_V1_dam_F",
    "Land_i_Shop_02_V1_dam_F",
    "Land_i_Shop_02_b_whiteblue_F",
    "Land_i_Shop_02_b_brown_F",
    "Land_i_Shop_02_b_white_F",
    "Land_i_Shop_02_b_yellow_F",
    "Land_i_Shop_02_b_pink_F",
    "Land_i_Shop_02_b_blue_F",
    "Land_Hotel_01_F",
    "Land_Hotel_02_F",
    "Land_School_01_F",
    "Land_Supermarket_01_F",
    "Land_Supermarket_01_malden_F",
    "Land_Hospital_main_F",
    "Land_Hospital_side1_F",
    "Land_Hospital_side2_F",
    "Land_HealthCenter_01_F",
    "Land_dp_mainFactory_F",
    "Land_dp_smallFactory_F",
    "Land_DPP_01_mainFactory_F",
    "Land_Warehouse_01_F",
    "Land_Warehouse_02_F",
    "Land_Warehouse_03_F",
    "Land_CarService_F",
    "Land_MultistoryBuilding_01_F",
    "Land_MultistoryBuilding_03_F",
    "Land_MultistoryBuilding_04_F",
    "Land_Church_01_F",
    "Land_Church_02_F",
    "Land_Church_03_F",
    "Land_Cathedral_01_F",
    "Land_Airport_01_terminal_F",
    "Land_Airport_02_terminal_F",
    "Land_Airport_left_F",
    "Land_Airport_right_F",
    "Land_Airport_01_hangar_F",
    "Land_Airport_02_hangar_left_F",
    "Land_Airport_02_hangar_right_F",
    "Land_Factory_Main_F",
    "Land_dp_bigTank_F",
    "Land_spp_Tower_F",
    "Land_SCF_01_boilerBuilding_F",
    "Land_SCF_01_generalBuilding_F",
    "Land_SCF_01_condenser_F",
    "Land_SCF_01_diffuser_F",
    "Land_SCF_01_clarifier_F",
    "Land_SCF_01_washer_F",
    "Land_SCF_01_crystallizer_F",
    "Land_SCF_01_crystallizerTowers_F",
    "Land_SCF_01_heap_bagasse_F",
    "Land_SCF_01_chimney_F",
    "Land_SY_01_crusher_F",
    "Land_SY_01_shiploader_F",
    "Land_SY_01_shiploader_arm_F",
    "Land_ContainerLine_01_F",
    "Land_ContainerLine_02_F",
    "Land_DryDock_01_end_F",
    "Land_DryDock_01_middle_F",
    "Land_StorageTank_01_large_F",
    "Land_Temple_Native_01_F",
    "Land_LightHouse_F",
    "Land_Lighthouse_03_red_F",
    "Land_Lighthouse_03_green_F",
    "Land_d_Shop_01_V1_F",
    "Land_House_Big_01_V1_ruins_F",
    "Land_House_Big_02_V1_ruins_F",
    "Land_GH_MainBuilding_left_F",
    "Land_GH_MainBuilding_middle_F",
    "Land_GH_MainBuilding_right_F",
    "Land_GH_House_1_F",
    "Land_Castle_01_tower_F",
    "Land_PoliceStation_01_F",
    "Land_Barn_04_F",
    "Land_Barn_03_large_F",
    "Land_Barn_01_grey_F",
    "Land_Barn_01_brown_F",
    "Land_Unfinished_Building_01_noLadder_F",
    "Land_House_1B01_F",
    "Land_House_2W05_F",
    "Land_ServiceHangar_01_L_F",
    "Land_ServiceHangar_01_R_F",
    "Land_Factory_02_F",
    "Land_CementWorks_01_grey_F",
    "Land_CementWorks_01_brick_F",
    "Land_ContainerLine_03_F",
    "Land_IndustrialShed_01_F",
    "Land_Smokestack_01_factory_F",
    "Land_Smokestack_01_F",
    "Land_CoalPlant_01_MainBuilding_F",
    "Land_Mine_01_warehouse_F",
    "Land_Rail_Station_Big_F",
    "Land_Cowshed_01_A_F",
    "Land_Church_04_lightblue_F",
    "Land_Church_04_yellow_F",
    "Land_Church_04_white_red_F",
    "Land_Church_04_red_F",
    "Land_Church_04_damaged_F",
    "Land_Church_04_yellow_damaged_F",
    "Land_ChurchRuin_01_F",
    "Land_Church_01_V1_F",
    "Land_WIP_F",
    "Land_i_Stone_HouseSmall_V1_F",
    "Land_i_Stone_HouseSmall_V2_F",
    "Land_i_Stone_HouseSmall_V3_F",
    "Land_i_Stone_HouseSmall_V1_dam_F",
    "Land_i_Stone_HouseSmall_V2_dam_F", 
    "Land_i_Stone_HouseSmall_V3_dam_F",
    "Land_d_Stone_HouseSmall_V1_F",
    "Land_i_House_Big_01_V2_dam_F",
    "Land_i_House_Big_01_V3_dam_F",
    "Land_u_House_Big_01_V1_dam_F",
    "Land_i_House_Big_02_V2_dam_F",
    "Land_i_House_Big_02_V3_dam_F",
    "Land_u_House_Big_02_V1_dam_F",
    "Land_i_Shop_01_V2_dam_F",
    "Land_i_Shop_01_V3_dam_F",
    "Land_u_Shop_01_V1_dam_F",
    "Land_i_Shop_02_V2_dam_F",
    "Land_i_Shop_02_V3_dam_F",
    "Land_u_Shop_02_V1_dam_F",
    "Land_i_House_Small_01_V3_dam_F",
    "Land_i_House_Small_02_V2_dam_F",
    "Land_i_House_Small_02_V3_dam_F",
    "Land_i_House_Small_03_V1_dam_F",
    "Land_i_Stone_HouseBig_V2_dam_F",
    "Land_i_Stone_HouseBig_V3_dam_F",
    "Land_Airport_Tower_dam_F",
    "Land_Unfinished_Building_01_F",
    "Land_Unfinished_Building_02_F",
    "Land_Unfinished_Building_01_ruins_F",
    "Land_Unfinished_Building_02_ruins_F",
    "Land_GH_Gazebo_F",
    "Land_GH_House_2_F",
    "Land_GarageShelter_01_F",
    "Land_House_Native_01_F",
    "Land_House_Native_02_F",
    "Land_i_House_Small_01_V1_F",
    "Land_i_House_Small_01_V2_F",
    "Land_i_House_Small_01_V3_F",
    "Land_i_House_Small_02_V1_F",
    "Land_i_House_Small_02_V2_F",
    "Land_i_House_Small_02_V3_F",
    "Land_i_House_Small_03_V1_F",
    "Land_i_House_Small_01_b_yellow_F",
    "Land_i_House_Small_01_b_pink_F",
    "Land_i_House_Small_01_b_whiteblue_F",
    "Land_i_House_Small_01_b_blue_F",
    "Land_i_House_Small_01_b_white_F",
    "Land_i_House_Small_01_b_brown_F",
    "Land_i_House_Small_02_b_whiteblue_F",
    "Land_i_House_Small_02_c_pink_F",
    "Land_i_House_Small_02_b_blue_F",
    "Land_i_House_Small_02_b_white_F",
    "Land_i_House_Small_02_b_pink_F",
    "Land_i_House_Small_02_b_brown_F",
    "Land_i_House_Small_02_b_yellow_F",
    "Land_i_House_Small_02_c_whiteblue_F",
    "Land_i_House_Small_02_c_blue_F",
    "Land_u_House_Small_01_V1_F",
    "Land_u_House_Small_02_V1_F",
    "Land_i_House_Small_01_V1_dam_F",
    "Land_i_House_Small_01_V2_dam_F",
    "Land_i_House_Small_02_V1_dam_F",
    "Land_d_House_Small_01_V1_F",
    "Land_d_House_Small_02_V1_F",
    "Land_u_House_Small_01_V1_dam_F",
    "Land_u_House_Small_02_V1_dam_F",
    "Land_House_Small_01_F",
    "Land_House_Small_02_F",
    "Land_House_Small_03_F",
    "Land_House_Small_04_F",
    "Land_House_Small_05_F",
    "Land_House_Small_06_F",
    "Land_SM_01_shed_F",
    "Land_SM_01_shed_unfinished_F"
];

// ============================================================================
// SIDE STRUCTURES (Satellite buildings - sheds, small huts, guard posts, garages)
// ============================================================================

// --- Military Side ---
private _sideMilitary = [
    "Cargo_Patrol_base_F",
    "Cargo_Tower_base_F",
    "Land_Cargo_House_V1_F",
    "Land_Cargo_House_V2_F",
    "Land_Cargo_House_V3_F",
    "Land_Cargo_House_V4_F",
    "Land_Cargo_House_V3_derelict_F",
    "Land_Cargo_Patrol_V2_ruins_F",
    "Land_Cargo_Tower_V3_derelict_F",
    "Land_Bunker_01_small_F",
    "Land_GuardHouse_01_F",
    "Land_Cargo_Patrol_V3_ruins_F",
    "Land_Bunker_02_right_F",
    "Land_Bunker_02_left_F",
    "Land_Bunker_02_double_F",
    "Land_Bunker_02_light_double_F",
    "Land_Bunker_02_light_left_F",
    "Land_Bunker_02_light_right_F",
    "Land_GuardHouse_02_F",
    "Land_GuardHouse_02_grey_F",
    "Land_GuardHouse_03_F",
    "Land_GuardBox_01_green_F",
    "Land_GuardBox_01_brown_F",
    "Land_Radar_01_antenna_F",
    "Land_Radar_01_cooler_F",
    "Land_Camp_House_01_brown_F",
    "Land_GuardTower_01_F",
    "Land_GuardTower_02_F",
    "Land_GuardBox_01_smooth_F",
    "Land_MobileRadar_01_radar_F",
    "Land_Medevac_house_V1_F",
    "Land_ConnectorTent_01_white_open_F",
    "Land_ConnectorTent_01_white_closed_F",
    "Land_ConnectorTent_01_white_cross_F",
    "Land_ConnectorTent_01_CSAT_brownhex_open_F",
    "Land_ConnectorTent_01_CSAT_brownhex_closed_F",
    "Land_ConnectorTent_01_CSAT_brownhex_cross_F",
    "Land_ConnectorTent_01_CSAT_greenhex_open_F",
    "Land_ConnectorTent_01_CSAT_greenhex_closed_F",
    "Land_ConnectorTent_01_CSAT_greenhex_cross_F",
    "Land_ConnectorTent_01_NATO_open_F",
    "Land_ConnectorTent_01_NATO_closed_F",
    "Land_ConnectorTent_01_NATO_cross_F",
    "Land_ConnectorTent_01_NATO_tropic_open_F",
    "Land_ConnectorTent_01_NATO_tropic_closed_F",
    "Land_ConnectorTent_01_NATO_tropic_cross_F",
    "Land_ConnectorTent_01_AAF_open_F",
    "Land_ConnectorTent_01_AAF_closed_F",
    "Land_ConnectorTent_01_AAF_cross_F",
    "Land_Dome_Small_WIP_F",
    "Land_Dome_Small_WIP2_F"
];

// --- Civilian Side ---
private _sideCivilian = [
    "Land_House_1W01_F",
    "Land_House_1W02_F",
    "Land_House_1W03_F",
    "Land_House_1W04_F",
    "Land_House_1W05_F",
    "Land_House_1W06_F",
    "Land_House_1W07_F",
    "Land_House_1W08_F",
    "Land_House_1W09_F",
    "Land_House_1W10_F",
    "Land_House_1W11_F",
    "Land_House_1W13_F",
    "Land_HouseRuin_Small_01_half_F",
    "Land_House_1W12_F",
    "Land_Slum_House01_F",
    "Land_Slum_House02_F",
    "Land_Slum_House03_F",
    "Land_Slum_01_F",
    "Land_Slum_02_F",
    "Land_Slum_03_F",
    "Land_Slum_04_F",
    "Land_Slum_05_F",
    "Land_i_Shed_Ind_F",
    "Land_i_Shed_Ind_old_F",
    "Land_Shed_Ind_old_ruins_F",
    "Land_u_Shed_Ind_F",
    "Land_i_Stone_Shed_V1_F",
    "Land_i_Stone_Shed_V2_F",
    "Land_i_Stone_Shed_V3_F",
    "Land_i_Stone_Shed_V1_dam_F",
    "Land_d_Stone_Shed_V1_F",
    "Land_i_Stone_Shed_01_b_raw_F",
    "Land_i_Stone_Shed_01_b_clay_F",
    "Land_i_Stone_Shed_01_b_white_F",
    "Land_i_Stone_Shed_01_c_white_F",
    "Land_i_Stone_Shed_01_c_clay_F",
    "Land_i_Stone_Shed_01_c_raw_F",
    "Land_Metal_Shed_F",
    "Land_Shed_01_F",
    "Land_Shed_02_F",
    "Land_Shed_03_F",
    "Land_Shed_04_F",
    "Land_Shed_05_F",
    "Land_Shed_06_F",
    "Land_Shed_07_F",
    "Land_Shed_09_F",
    "Land_Shed_10_F",
    "Land_Shed_11_F",
    "Land_Shed_12_F",
    "Land_Shed_14_F",
    "Land_Shed_05_ruins_F",
    "Land_Shed_08_brown_F",
    "Land_Shed_08_grey_F",
    "Land_Workshop_01_F",
    "Land_Workshop_02_F",
    "Land_Workshop_03_F",
    "Land_Workshop_04_F",
    "Land_Workshop_05_F",
    "Land_i_Garage_V1_F",
    "Land_i_Garage_V2_F",
    // "Land_GarageShelter_01_F",
    "Land_GarageRow_01_small_F",
    "Land_WaterStation_01_F",
    "Land_PowerStation_01_F",
    "Land_Substation_01_F",
    "Land_i_Addon_02_V1_F",
    "Land_i_Addon_03_V1_F",
    "Land_i_Addon_03mid_V1_F",
    "Land_i_Addon_04_V1_F",
    "Land_u_Addon_01_V1_F",
    "Land_u_Addon_02_V1_F",
    "Land_Addon_04_F",
    "Land_d_Addon_02_V1_F",
    "Land_Chapel_Small_V1_F",
    "Land_Chapel_Small_V2_F",
    "Land_Chapel_V1_F",
    "Land_Chapel_V2_F",
    "Land_Chapel_02_white_F",
    "Land_Chapel_02_yellow_F",
    "Land_Chapel_02_white_damaged_F",
    "Land_Chapel_02_yellow_damaged_F",
    "Land_Chapel_02_white_ruins_F",
    "Land_Chapel_02_yellow_ruins_F",
    "Land_Church_05_F",
    "Land_VillageStore_01_F",
    "Land_FuelStation_Build_F",
    "Land_FuelStation_Shed_F",
    "Land_FuelStation_01_shop_F",
    "Land_FuelStation_01_workshop_F",
    "Land_FuelStation_02_workshop_F",
    "Land_FuelStation_03_shop_F",
    // "Land_Unfinished_Building_01_F",
    // "Land_Unfinished_Building_02_F",
    // "Land_Unfinished_Building_01_ruins_F",
    // "Land_Unfinished_Building_02_ruins_F",
    "Land_d_Shop_02_V1_F",
    // "Land_Shop_01_V1_ruins_F",
    // "Land_Shop_02_V1_ruins_F",
    "Land_House_Small_01_V1_ruins_F",
    "Land_House_Small_02_V1_ruins_F",
    "Land_Stone_HouseBig_V1_ruins_F",
    "Land_Stone_Shed_V1_ruins_F",
    "Land_Garage_V1_ruins_F",
    "Land_Addon_02_V1_ruins_F",
    "Land_Addon_04_V1_ruins_F",
    "Land_i_Windmill01_F",
    "Land_d_Windmill01_F",
    "Land_Lighthouse_small_F",
    "Land_Crane_F",
    "Land_ContainerCrane_01_F",
    "Land_MobileCrane_01_F",
    "Land_MobileCrane_01_hook_F",
    // "Land_GH_Gazebo_F",
    // "Land_GH_House_2_F",
    "Land_Pier_F",
    "Land_Pier_small_F",
    "Land_nav_pier_m_F",
    "Land_PierWooden_01_dock_F",
    "Land_PierWooden_01_hut_F",
    "Land_PierWooden_01_platform_F",
    "Land_PierWooden_01_16m_F",
    "Land_PierWooden_01_10m_noRails_F",
    "Land_PierWooden_02_16m_F",
    "Land_PierWooden_02_30deg_F",
    "Land_PierWooden_02_ladder_F",
    "Land_PierWooden_02_barrel_F",
    "Land_PierWooden_02_hut_F",
    "Land_Bridge_HighWay_PathLod_F",
    "Land_Bridge_01_PathLod_F",
    "Land_Bridge_Asphalt_PathLod_F",
    "Land_Bridge_Concrete_PathLod_F",
    "Land_Canal_Wall_Stairs_F",
    "Land_Track_01_bridge_F",
    "Land_Stadium_p4_F",
    "Land_Stadium_p5_F",
    "Land_Stadium_p9_F",
    "Land_Barn_02_F",
    "Land_Barn_03_small_F",
    "Land_Cowshed_01_B_F",
    "Land_Cowshed_01_C_F",
    "Land_FeedShack_01_F",
    "Land_Sawmill_01_F",
    "Land_Sawmill_01_illuminati_tower_F",
    "Land_DeerStand_02_F",
    "Land_dp_bigTank_old_F",
    "Land_Rail_Warehouse_Small_F",
    "Land_StorageTank_01_small_F",
    "Land_SCF_01_feeder_F",
    "Land_SCF_01_storageBin_big_F",
    "Land_SCF_01_storageBin_medium_F",
    "Land_SCF_01_storageBin_small_F",
    "Land_SY_01_reclaimer_F",
    "Land_SY_01_conveyor_end_F",
    // "Land_Shop_Town_02_F",
    "Land_Mausoleum_01_F",
    "Land_FireEscape_01_tall_F",
    "Land_FireEscape_01_short_F",
    "Land_HaulTruck_01_abandoned_F",
    "Land_MiningShovel_01_abandoned_F",
    "Land_CastleRuins_01_bastion_F",
    "Land_WoodenWall_04_s_d_5m_F",
    "Land_BrickWall_03_l_gate_F",
    "Land_Highway_Pillar_01_garage_F",
    "Land_Caravan_01_rust_F",
    "Land_u_Addon_01_V1_dam_F",
    "Land_i_Garage_V1_dam_F",
    "Land_i_Garage_V2_dam_F",
    "Land_i_Stone_Shed_V2_dam_F",
    "Land_i_Stone_Shed_V3_dam_F",
    "Land_Kiosk_blueking_F",
    "Land_Kiosk_gyros_F",
    "Land_Kiosk_papers_F",
    "Land_Kiosk_redburger_F"
];

// ============================================================================
// PER-MAP EXCLUSIONS
// ============================================================================
private _mapExclusions = createHashMapFromArray [
    ["Altis", [
        "Land_Cargo_Patrol_V2_F",
        "Land_Cargo_Tower_V2_F",
        "Land_Cargo_HQ_V2_F",
        "Land_Cargo_House_V2_F"
    ]],
    ["Stratis", []],
    ["Tanoa", []],
    ["Malden", []],
    ["Enoch", [
        "Land_DeerStand_02_F"
    ]]
];

private _currentMap = worldName;
private _exclusions = _mapExclusions getOrDefault [_currentMap, []];

if (_exclusions isNotEqualTo []) then {
    diag_log format ["DSC: fnc_getStructureTypes - Map %1: %2 exclusion rules active", _currentMap, count _exclusions];
};

// ============================================================================
// FUNCTIONAL CATEGORIES — what the building IS (for location tagging)
// ============================================================================
// Each structure gets ONE primary functional category. Structures not listed
// in any category are still valid for garrison but don't contribute to
// functional tagging. When adding a structure to main/side above, also add
// it to the appropriate functional category below.

private _func_residential = [
    // Main — Mediterranean Big Houses
    "Land_i_House_Big_01_V1_F", "Land_i_House_Big_01_V2_F", "Land_i_House_Big_01_V3_F",
    "Land_i_House_Big_02_V1_F", "Land_i_House_Big_02_V2_F", "Land_i_House_Big_02_V3_F",
    "Land_House_Big_05_F",
    // Main — Malden Big Houses
    "Land_i_House_Big_01_b_yellow_F", "Land_i_House_Big_01_b_blue_F",
    "Land_i_House_Big_01_b_pink_F", "Land_i_House_Big_01_b_white_F",
    "Land_i_House_Big_01_b_whiteblue_F", "Land_i_House_Big_01_b_brown_F",
    "Land_i_House_Big_02_b_pink_F", "Land_i_House_Big_02_b_brown_F",
    "Land_i_House_Big_02_b_whiteblue_F", "Land_i_House_Big_02_b_blue_F",
    "Land_i_House_Big_02_b_yellow_F", "Land_i_House_Big_02_b_white_F",
    // Main — Urban Big Houses
    "Land_u_House_Big_01_V1_F", "Land_u_House_Big_02_V1_F",
    // Main — Stone Houses (Big)
    "Land_i_Stone_HouseBig_V1_F", "Land_i_Stone_HouseBig_V2_F",
    "Land_i_Stone_HouseBig_V3_F", "Land_i_Stone_House_Big_01_b_clay_F",
    // Main — Damaged Big Houses
    "Land_i_House_Big_01_V1_dam_F", "Land_i_House_Big_02_V1_dam_F",
    "Land_i_House_Big_01_V2_dam_F", "Land_i_House_Big_01_V3_dam_F",
    "Land_d_House_Big_01_V1_F", "Land_d_House_Big_02_V1_F",
    "Land_i_Stone_HouseBig_V1_dam_F", "Land_d_Stone_HouseBig_V1_F",
    "Land_u_House_Big_01_V1_dam_F", "Land_i_House_Big_02_V2_dam_F",
    "Land_i_House_Big_02_V3_dam_F", "Land_u_House_Big_02_V1_dam_F",
    "Land_i_Shop_01_V2_dam_F", "Land_i_Shop_01_V3_dam_F", "Land_u_Shop_01_V1_dam_F",
    "Land_i_Shop_02_V2_dam_F", "Land_i_Shop_02_V3_dam_F", "Land_u_Shop_02_V1_dam_F",
    "Land_i_House_Small_01_V3_dam_F", "Land_i_House_Small_02_V2_dam_F", "Land_i_House_Small_02_V3_dam_F",
    "Land_i_House_Small_03_V1_dam_F", "Land_i_Stone_HouseBig_V2_dam_F", "Land_i_Stone_HouseBig_V3_dam_F",
    // Main — Tanoa Big Houses
    "Land_House_Big_01_F", "Land_House_Big_02_F",
    "Land_House_Big_03_F", "Land_House_Big_04_F",
    // Main — Livonia Houses (2-wide)
    "Land_House_2W01_F", "Land_House_2W02_F", "Land_House_2W03_F", "Land_House_2W04_F",
    "Land_House_2B01_F", "Land_House_2B02_F", "Land_House_2B03_F", "Land_House_2B04_F",
    // Main — House Ruins (Big)
    "Land_HouseRuin_Big_01_F", "Land_HouseRuin_Big_01_half_F",
    "Land_HouseRuin_Big_02_F", "Land_HouseRuin_Big_02_half_F",
    "Land_HouseRuin_Big_03_half_F",
    // Main — Multi-story
    "Land_MultistoryBuilding_01_F", "Land_MultistoryBuilding_03_F", "Land_MultistoryBuilding_04_F",
    // Main — Estate / Manor
    "Land_GH_MainBuilding_left_F", "Land_GH_MainBuilding_middle_F",
    "Land_GH_MainBuilding_right_F", "Land_GH_House_1_F",
    // Main — Castle
    "Land_Castle_01_tower_F",
    // Main — Livonia
    "Land_House_1B01_F", "Land_House_2W05_F",
    // Main — Big Ruins
    "Land_House_Big_01_V1_ruins_F", "Land_House_Big_02_V1_ruins_F",
    // Side — Mediterranean Small Houses
    "Land_i_House_Small_01_V1_F", "Land_i_House_Small_01_V2_F", "Land_i_House_Small_01_V3_F",
    "Land_i_House_Small_02_V1_F", "Land_i_House_Small_02_V2_F", "Land_i_House_Small_02_V3_F",
    "Land_i_House_Small_03_V1_F",
    // Side — Malden Small Houses
    "Land_i_House_Small_01_b_yellow_F", "Land_i_House_Small_01_b_pink_F",
    "Land_i_House_Small_01_b_whiteblue_F", "Land_i_House_Small_01_b_blue_F",
    "Land_i_House_Small_01_b_white_F", "Land_i_House_Small_01_b_brown_F",
    "Land_i_House_Small_02_b_whiteblue_F", "Land_i_House_Small_02_c_pink_F",
    "Land_i_House_Small_02_b_blue_F", "Land_i_House_Small_02_b_white_F",
    "Land_i_House_Small_02_b_pink_F", "Land_i_House_Small_02_b_brown_F",
    "Land_i_House_Small_02_b_yellow_F", "Land_i_House_Small_02_c_whiteblue_F",
    "Land_i_House_Small_02_c_blue_F",
    // Side — Urban Small Houses
    "Land_u_House_Small_01_V1_F", "Land_u_House_Small_02_V1_F",
    // Side — Stone Houses (Small)
    "Land_i_Stone_HouseSmall_V1_F", "Land_i_Stone_HouseSmall_V2_F", "Land_i_Stone_HouseSmall_V3_F",
    // Side — Damaged Small Houses
    "Land_i_House_Small_01_V1_dam_F", "Land_i_House_Small_01_V2_dam_F",
    "Land_i_House_Small_02_V1_dam_F", "Land_d_House_Small_01_V1_F",
    "Land_d_House_Small_02_V1_F", "Land_i_Stone_HouseSmall_V1_dam_F",
    "Land_i_Stone_HouseSmall_V2_dam_F", "Land_i_Stone_HouseSmall_V3_dam_F",
    "Land_d_Stone_HouseSmall_V1_F", "Land_u_House_Small_01_V1_dam_F",
    "Land_u_House_Small_02_V1_dam_F", "Land_i_Stone_Shed_V2_dam_F",
    "Land_i_Stone_Shed_V3_dam_F",
    // Side — Tanoa Small Houses
    "Land_House_Small_01_F", "Land_House_Small_02_F", "Land_House_Small_03_F",
    "Land_House_Small_04_F", "Land_House_Small_05_F", "Land_House_Small_06_F",
    "Land_House_Native_01_F", "Land_House_Native_02_F",
    // Side — Livonia (1-wide)
    "Land_House_1W01_F", "Land_House_1W02_F", "Land_House_1W03_F", "Land_House_1W04_F",
    "Land_House_1W05_F", "Land_House_1W06_F", "Land_House_1W07_F", "Land_House_1W08_F",
    "Land_House_1W09_F", "Land_House_1W10_F", "Land_House_1W11_F", "Land_House_1W13_F",
    "Land_House_1W12_F",
    // Side — Slums
    "Land_Slum_House01_F", "Land_Slum_House02_F", "Land_Slum_House03_F",
    "Land_Slum_01_F", "Land_Slum_02_F", "Land_Slum_03_F",
    "Land_Slum_04_F", "Land_Slum_05_F",
    // Side — Addons (attached rooms)
    "Land_i_Addon_02_V1_F", "Land_i_Addon_03_V1_F", "Land_i_Addon_03mid_V1_F",
    "Land_i_Addon_04_V1_F", "Land_u_Addon_01_V1_F", "Land_u_Addon_02_V1_F",
    "Land_Addon_04_F", "Land_d_Addon_02_V1_F",
    // Side — Stone Sheds (Mediterranean outbuildings)
    "Land_i_Stone_Shed_V1_F", "Land_i_Stone_Shed_V2_F", "Land_i_Stone_Shed_V3_F",
    "Land_i_Stone_Shed_V1_dam_F", "Land_d_Stone_Shed_V1_F",
    "Land_i_Stone_Shed_01_b_raw_F", "Land_i_Stone_Shed_01_b_clay_F",
    "Land_i_Stone_Shed_01_b_white_F", "Land_i_Stone_Shed_01_c_white_F",
    "Land_i_Stone_Shed_01_c_clay_F", "Land_i_Stone_Shed_01_c_raw_F",
    // Side — Private Garages
    "Land_i_Garage_V1_F", "Land_i_Garage_V2_F", "Land_GarageShelter_01_F",
    // Side — Estate/Manor
    "Land_GH_Gazebo_F", "Land_GH_House_2_F",
    // Side — Caravan
    "Land_Caravan_01_rust_F",
    // Side — Small Ruins
    "Land_HouseRuin_Small_01_half_F",
    "Land_House_Small_01_V1_ruins_F", "Land_House_Small_02_V1_ruins_F",
    "Land_Stone_HouseBig_V1_ruins_F", "Land_Stone_Shed_V1_ruins_F",
    "Land_Garage_V1_ruins_F", "Land_Addon_02_V1_ruins_F", "Land_Addon_04_V1_ruins_F",
    "Land_u_Addon_01_V1_dam_F", "Land_i_Garage_V1_dam_F", "Land_i_Garage_V2_dam_F"
];

private _func_commercial = [
    // Main — Office Building
    "Land_Offices_01_V1_F",
    // Main — Shops (all variants)
    "Land_i_Shop_01_V1_F", "Land_i_Shop_01_V2_F", "Land_i_Shop_01_V3_F",
    "Land_i_Shop_02_V1_F", "Land_i_Shop_02_V2_F", "Land_i_Shop_02_V3_F",
    "Land_u_Shop_01_V1_F", "Land_u_Shop_02_V1_F",
    "Land_Shop_City_01_F", "Land_Shop_City_02_F", "Land_Shop_City_04_F",
    "Land_Shop_City_05_F", "Land_Shop_City_06_F", "Land_Shop_City_07_F",
    "Land_Shop_Town_01_F", "Land_Shop_Town_03_F", "Land_Shop_Town_05_F",
    "Land_i_Shop_01_V1_dam_F", "Land_i_Shop_02_V1_dam_F",
    "Land_Kiosk_blueking_F", "Land_Kiosk_gyros_F", "Land_Kiosk_papers_F",
    "Land_Kiosk_redburger_F",
    // Main — Malden Shops
    "Land_i_Shop_02_b_whiteblue_F", "Land_i_Shop_02_b_brown_F",
    "Land_i_Shop_02_b_white_F", "Land_i_Shop_02_b_yellow_F",
    "Land_i_Shop_02_b_pink_F", "Land_i_Shop_02_b_blue_F",
    // Main — Hotels / Schools / Supermarkets
    "Land_Hotel_01_F", "Land_Hotel_02_F",
    "Land_School_01_F",
    "Land_Supermarket_01_F", "Land_Supermarket_01_malden_F",
    // Main — Garage Complexes (commercial)
    "Land_GarageRow_01_large_F", "Land_GarageOffice_01_F",
    // Main — Damaged Shops
    "Land_d_Shop_01_V1_F", "Land_WIP_F",
    // Side — Small Commercial
    "Land_VillageStore_01_F",
    "Land_FuelStation_01_shop_F", "Land_FuelStation_03_shop_F",
    "Land_FuelStation_01_workshop_F", "Land_FuelStation_02_workshop_F",
    "Land_GarageRow_01_small_F",
    "Land_Shop_Town_02_F",
    // Side — Damaged Small Shops
    "Land_d_Shop_02_V1_F", "Land_Shop_01_V1_ruins_F", "Land_Shop_02_V1_ruins_F"
];

private _func_industrial = [
    // Main — Factories
    "Land_dp_mainFactory_F", "Land_dp_smallFactory_F", "Land_DPP_01_mainFactory_F",
    "Land_Factory_Main_F", "Land_Factory_02_F",
    // Main — Warehouses
    "Land_Warehouse_01_F", "Land_Warehouse_02_F", "Land_Warehouse_03_F",
    "Land_SM_01_shed_F", "Land_SM_01_shed_unfinished_F",
    // Main — Car Service
    "Land_CarService_F",
    // Main — Sugar Cane Factory (Tanoa)
    "Land_SCF_01_boilerBuilding_F", "Land_SCF_01_generalBuilding_F",
    "Land_SCF_01_condenser_F", "Land_SCF_01_diffuser_F",
    "Land_SCF_01_clarifier_F", "Land_SCF_01_washer_F",
    "Land_SCF_01_crystallizer_F", "Land_SCF_01_crystallizerTowers_F",
    "Land_SCF_01_heap_bagasse_F", "Land_SCF_01_chimney_F",
    // Main — Heavy Industry
    "Land_dp_bigTank_F", "Land_spp_Tower_F",
    "Land_CementWorks_01_grey_F", "Land_CementWorks_01_brick_F",
    "Land_IndustrialShed_01_F",
    "Land_Smokestack_01_factory_F", "Land_Smokestack_01_F",
    "Land_CoalPlant_01_MainBuilding_F",
    "Land_Mine_01_warehouse_F",
    "Land_StorageTank_01_large_F",
    // Side — Industrial Sheds
    "Land_i_Shed_Ind_F", "Land_i_Shed_Ind_old_F",
    "Land_Shed_Ind_old_ruins_F", "Land_u_Shed_Ind_F",
    "Land_Metal_Shed_F",
    // Side — Workshops
    "Land_Workshop_01_F", "Land_Workshop_02_F", "Land_Workshop_03_F",
    "Land_Workshop_04_F", "Land_Workshop_05_F",
    // Side — Small Industrial
    "Land_dp_bigTank_old_F",
    "Land_Rail_Warehouse_Small_F",
    "Land_StorageTank_01_small_F",
    // Side — Sugar Cane Factory (side)
    "Land_SCF_01_feeder_F",
    "Land_SCF_01_storageBin_big_F", "Land_SCF_01_storageBin_medium_F",
    "Land_SCF_01_storageBin_small_F",
    // Side — Mining
    "Land_HaulTruck_01_abandoned_F", "Land_MiningShovel_01_abandoned_F"
];

private _func_agricultural = [
    // Main — Barns (Large)
    "Land_Barn_04_F", "Land_Barn_03_large_F",
    "Land_Barn_01_grey_F", "Land_Barn_01_brown_F",
    // Main — Farming (Large)
    "Land_Cowshed_01_A_F",
    // Side — Barns (Small)
    "Land_Barn_02_F", "Land_Barn_03_small_F",
    "Land_Cowshed_01_B_F", "Land_Cowshed_01_C_F",
    "Land_FeedShack_01_F",
    // Side — Sawmill
    "Land_Sawmill_01_F", "Land_Sawmill_01_illuminati_tower_F",
    // Side — Windmills
    "Land_i_Windmill01_F", "Land_d_Windmill01_F"
];

private _func_medical = [
    // Main — Hospitals
    "Land_Hospital_main_F", "Land_Hospital_side1_F", "Land_Hospital_side2_F",
    "Land_HealthCenter_01_F",
    // Military crossover
    "Land_Medevac_HQ_V1_F", "Land_Medevac_house_V1_F"
];

private _func_religious = [
    // Main — Churches
    "Land_Church_01_F", "Land_Church_02_F", "Land_Church_03_F",
    "Land_Cathedral_01_F", "Land_Church_01_V1_F",
    "Land_Temple_Native_01_F",
    // Main — Livonia Churches
    "Land_Church_04_lightblue_F", "Land_Church_04_yellow_F",
    "Land_Church_04_white_red_F", "Land_Church_04_red_F",
    "Land_Church_04_damaged_F", "Land_Church_04_yellow_damaged_F",
    "Land_ChurchRuin_01_F",
    // Side — Chapels
    "Land_Chapel_Small_V1_F", "Land_Chapel_Small_V2_F",
    "Land_Chapel_V1_F", "Land_Chapel_V2_F",
    "Land_Chapel_02_white_F", "Land_Chapel_02_yellow_F",
    "Land_Chapel_02_white_damaged_F", "Land_Chapel_02_yellow_damaged_F",
    "Land_Chapel_02_white_ruins_F", "Land_Chapel_02_yellow_ruins_F",
    "Land_Church_05_F",
    // Side — Mausoleum
    "Land_Mausoleum_01_F"
];

private _func_infrastructure = [
    // Side — Utility
    "Land_WaterStation_01_F", "Land_PowerStation_01_F", "Land_Substation_01_F",
    // Side — Fuel Infrastructure
    "Land_FuelStation_Build_F", "Land_FuelStation_Shed_F",
    // Main — Lighthouses
    "Land_LightHouse_F", "Land_Lighthouse_03_red_F", "Land_Lighthouse_03_green_F",
    // Side — Small Lighthouse
    "Land_Lighthouse_small_F",
    // Main — Rail
    "Land_Rail_Station_Big_F"
];

private _func_port = [
    // Main — Shipyard / Port (Tanoa)
    "Land_SY_01_crusher_F", "Land_SY_01_shiploader_F", "Land_SY_01_shiploader_arm_F",
    "Land_ContainerLine_01_F", "Land_ContainerLine_02_F", "Land_ContainerLine_03_F",
    "Land_DryDock_01_end_F", "Land_DryDock_01_middle_F",
    // Side — Piers
    "Land_Pier_F", "Land_Pier_small_F", "Land_nav_pier_m_F",
    "Land_PierWooden_01_dock_F", "Land_PierWooden_01_hut_F",
    "Land_PierWooden_01_platform_F", "Land_PierWooden_01_16m_F",
    "Land_PierWooden_01_10m_noRails_F",
    "Land_PierWooden_02_16m_F", "Land_PierWooden_02_30deg_F",
    "Land_PierWooden_02_ladder_F", "Land_PierWooden_02_barrel_F",
    "Land_PierWooden_02_hut_F",
    // Side — Shipyard (side components)
    "Land_SY_01_reclaimer_F", "Land_SY_01_conveyor_end_F",
    // Side — Cranes
    "Land_Crane_F", "Land_ContainerCrane_01_F",
    "Land_MobileCrane_01_F", "Land_MobileCrane_01_hook_F"
];

private _func_airport = [
    // Main — Civilian Terminals & Hangars
    "Land_Airport_01_terminal_F", "Land_Airport_02_terminal_F",
    "Land_Airport_left_F", "Land_Airport_right_F",
    "Land_Airport_01_hangar_F",
    "Land_Airport_02_hangar_left_F", "Land_Airport_02_hangar_right_F",
    // Main — Service Hangars
    "Land_ServiceHangar_01_L_F", "Land_ServiceHangar_01_R_F",
    // Military crossover — Towers & Hangars
    "Land_Airport_Tower_F",
    "Land_Airport_01_controlTower_F", "Land_Airport_02_controlTower_F",
    "Land_ControlTower_01_F", "Land_ControlTower_02_F",
    "Land_Hangar_F", "Land_Airport_Tower_dam_F"
];

private _func_law_enforcement = [
    "Land_PoliceStation_01_F"
];

// Build functional categories hashmap
private _functionalCategories = createHashMapFromArray [
    ["residential", _func_residential],
    ["commercial", _func_commercial],
    ["industrial", _func_industrial],
    ["agricultural", _func_agricultural],
    ["medical", _func_medical],
    ["religious", _func_religious],
    ["infrastructure", _func_infrastructure],
    ["port", _func_port],
    ["airport", _func_airport],
    ["law_enforcement", _func_law_enforcement]
];

// Build reverse lookup: class name -> category (O(1) lookup during scan)
private _functionalLookup = createHashMap;
{
    private _category = _x;
    { _functionalLookup set [_x, _category] } forEach _y;
} forEach _functionalCategories;

// ============================================================================
// NON-OCCUPIABLE TYPES — structures with no buildingPos that characterize
// a location's function. Used for tagging only, not AI placement.
// EXPAND THROUGH TESTING: add classes as you find them on each map.
// ============================================================================
private _nonOccupiable = createHashMapFromArray [
    ["industrial", [
        "Land_dp_tankSmall_F",
        "Land_dp_tankRound_F",
        "Land_IndPipe1_ground_F",
        "Land_IndPipe1_20m_F",
        "Land_Ind_SiloVelke_F",
        "Land_ReservoirTank_V1_F",
        "Land_Tank_rust_F",
        "Land_Ind_TankSmall_F",
        "Land_Ind_TankSmall2_EP1_F"
    ]],
    ["agricultural", [
        "Land_Greenhouse_01_F",
        "Land_Silo_01_F",
        "Land_Silo_Wheat_F"
    ]],
    ["infrastructure", [
        "Land_Communication_F",
        "Land_TTowerBig_1_F",
        "Land_TTowerBig_2_F",
        "Land_PowerPoleWooden_F",
        "Land_FuelStation_Feed_F"
    ]],
    ["port", [
        "Land_NavigLight_F",
        "Land_Bollard_01_F"
    ]],
    ["military", [
        "Land_HelipadCircle_F",
        "Land_HelipadSquare_F",
        "Land_HelipadRescue_F",
        "Land_HBarrier_Big_F",
        "Land_HBarrier_1_F",
        "Land_Dome_Big_F",
        "Land_Dome_Small_F"
    ]]
];

// Build non-occupiable reverse lookup
private _nonOccupiableLookup = createHashMap;
{
    private _category = _x;
    { _nonOccupiableLookup set [_x, _category] } forEach _y;
} forEach _nonOccupiable;

// ============================================================================
// COMBINE AND RETURN
// ============================================================================
private _result = createHashMapFromArray [
    ["main", _mainMilitary + _mainCivilian],
    ["side", _sideMilitary + _sideCivilian],
    ["mainMilitary", _mainMilitary],
    ["mainCivilian", _mainCivilian],
    ["sideMilitary", _sideMilitary],
    ["sideCivilian", _sideCivilian],
    ["exclusions", _exclusions],
    ["functionalCategories", _functionalCategories],
    ["functionalLookup", _functionalLookup],
    ["nonOccupiable", _nonOccupiable],
    ["nonOccupiableLookup", _nonOccupiableLookup]
];

_result
