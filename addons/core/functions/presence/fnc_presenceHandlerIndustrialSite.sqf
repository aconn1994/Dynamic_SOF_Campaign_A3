/*
 * Function: DSC_core_fnc_presenceHandlerIndustrialSite
 * Description:
 *     Microzone handler (Sprint D.5) for industrial-tagged sites.
 *
 *     Spawn pass:
 *       1. 1-3 civilian workers (resolved via classMix)
 *       2. Projection-driven guard (typeMult 1.0)
 *       3. Projection-driven patrol (typeMult 1.0)
 *       4. Irregular fallback (30%) when no controller in range
 *
 * Arguments:
 *     0: _zone <HASHMAP>
 *
...
// ----- Civilians -----
private _civCount = 3 + floor random 3; // 3-5
...
// ----- Projection-driven guard/patrol -----
...
// Irregular fallback (no controller in range)
if (!_hasController && {(_proj get "irregularFallback")} && {random 1 < 0.55}) then {
    ([["irregulars", "opForPartner"]] call _fnc_pickFootGroups) params ["_pool", ""];
    if (_pool isNotEqualTo []) then {
        private _pPool = [_pool] call DSC_core_fnc_filterPatrolGroups;
        if (_pPool isEqualTo []) then { _pPool = _pool };
        private _p = [_pos, _pPool, east, createHashMapFromArray [
            ["size",         [4, 5]],
            ["radius",       250],
