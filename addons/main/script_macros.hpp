...
#define DFUNC(var1) TRIPLES(ADDON,fnc,var1)
  ...
  #define PREP_SUB(folder,fncName) DFUNC(fncName) = compile preprocessFileLineNumbers QPATHTOF(functions\folder\DOUBLES(fnc,fncName).sqf)
  ...
  #define PREP_SUB(folder,fncName) [QPATHTOF(functions\folder\DOUBLES(fnc,fncName).sqf), QFUNC(fncName)] call CBA_fnc_compileFunction
